...
const args = new DataView(args_buffer); 
...
args.setFloat64(8, fibre_content, native_endianness);
...
const url = `${base_url}/compute/elastic_modules_for_unidirectional_composite`;
const req = new XMLHttpRequest();
req.open('POST', url);
req.responseType = 'arraybuffer';
req.onload = (_event) => {
        //const status_code = req.status;
        //const console.log(`status_code = ${status_code}`);     
        const array_buffer = req.response;
        if (array_buffer) {
                const resp_view = new Float64Array(array_buffer);
